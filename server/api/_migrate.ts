export default defineEventHandler(async (event) => {
  ...
  // ── Seed: menus (9 dimsum variants) ──
  const skus: Array<[string, string]> = [
    ['hisitkau', 'Hisitkau'],
  ...
  for (const [id, name] of skus) {
    await sql`INSERT INTO menus (id, name, unit, category) VALUES (${id}, ${name}, 'pcs', 'dimsum') ON CONFLICT (id) DO NOTHING`
  }
